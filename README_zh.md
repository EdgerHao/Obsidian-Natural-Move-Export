...
### 📥 将外部文件粘贴到 Obsidian
在访达、文件资源管理器或 Linux 文件管理器中复制文件或文件夹，然后选中 Obsidian 文件夹并按 `Command + V`（macOS）或 `Ctrl + V`（Windows/Linux）；也可以右键单击文件夹并选择 **Natural move: 粘贴外部文件到此处**。遇到同名项目时会自动生成新名称，不会覆盖已有文件。
